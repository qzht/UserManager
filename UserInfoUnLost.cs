...
{
    public partial class UserInfoUnLost : Form
    {
        public UserInfoUnLost()
        {
            InitializeComponent();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                ...
                {
                    MessageBox.Show("该卡状态正常，无需解挂", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                db.NonQuery(" update MerInfo set isShuiCardLost=1 where ID= " + dt.Rows[0]["ID"].ToString() + "");
           
                db.NonQuery("insert into U_AuditLog(OpeType,UserID,MerNum,CarType) values('解挂','" + Common.UserId + "','" + MerNum + "',0)");
                Common.WriteOpearateLog(Common.UserId, "商户" + MerNum + txtUserName.Text + "解挂");
                MessageBox.Show("解挂成功", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else {
                MessageBox.Show("解挂失败", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UserInfoUnLost_Load(object sender, EventArgs e)
        {
